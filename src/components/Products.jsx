  ...
  return (
    <div className='body-wrapper max-w-screen-2xl mx-auto'>
        ...
        <div className='mt-36 text-center'>
          ...
          <ul className='flex gap-3 justify-center pt-10'>
            ...
            {filters && filters.map(filter =>
              <li className={`cursor-pointer w-auto  p-3  rounded-lg ${clickedFilter === filter? "bg-brand text-white":""}`}onClick={()=>handleFilterSelection(filter)}>{filter}</li>
            )} */}
            {filters && (
              <>
      ...
      <ul className='grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-8 p-16 mt-4 my-12  h-auto'>
        ...
        {category.length >=1 && products && clickedFilter === 'All'? 
          products
            .filter(item => item.category.includes(category))
            .map(item => (
