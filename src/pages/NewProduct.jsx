...
export default function NewProduct() {
  ...
  const handleChange = (ev)=>{
    // ev.target.files는 input type="file"을 쓰면 디폴트로 제공해주는 애들임.
    // ev.target.value는 사용자 Input임. 즉, 사용자가 넣은 값을 value로, name을 key로 저장하라는 것임. 
    const { name, value, files } = ev.target;
    if(name === "file"){
      setFile(files && [...files]); // if(files){setFile(files[0])}의 의미임
      return;
    }
    console.log('파일입니까?', [name],':', value)
    ...
    <section className='w-full text-center body-wrapper mt-28'>
      ...
      { file && <img className="w-96 mx-auto mb-2" src={URL.createObjectURL(file[0])} alt='local file'/>}
        <form onSubmit={handleSubmit} className='flex flex-col px-12'>
          ...
          <input type="text" name="options" value={product.options ?? ''} placeholder='Options(Separated by comma (,))' required onChange={handleChange} />
